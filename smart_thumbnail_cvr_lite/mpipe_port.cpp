...
cv::Mat mpipe_port_getNextFrame(std::vector<cv::Point>& roiCoords, std::vector<std::vector<cv::Point>>& motionblobs) {
    ...
    if((isMotionFilter && !smTnInstance -> waitForNextMotionFrame()) || (smTnInstance->clipEnd)) return cv_frame;
    ...
    std::unique_lock<std::mutex> lock(smTnInstance -> QMutex);
#else
