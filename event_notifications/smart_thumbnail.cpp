...
void SmartThumbnail::onMsgProcessFrame(rtMessageHeader const* hdr, uint8_t const* buff, uint32_t n, void* closure)
{
	...
	smartThInst -> ProcessFrameMetadata(sm, motionFlags);

	rtMessage_Release(m);
}

void SmartThumbnail::ProcessFrameMetadata(SmarttnMetadata_thumb sm, int motionFlags)
{
...
STH_STATUS SmartThumbnail::rtMsgInit()
{
	...
	rtConnection_AddListener(connectionRecv, "RDKC.SMARTTN.DOICHANGE",onMsgDOIChanged, NULL);
#endif
...
STH_STATUS SmartThumbnail::uploadPayload()
{
	...
	if ((response_code >= RDKC_HTTP_RESPONSE_OK) && (response_code < RDKC_HTTP_RESPONSE_REDIRECT_START)) {
		...
		eventDOIquietTimeStart = currTime.tv_sec;
#ifdef ENABLE_TEST_HARNESS
		stnUploadTime = currTstamp;
#else
		stnUploadTime = currTime.tv_sec;
